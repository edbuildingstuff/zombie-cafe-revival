...
- *(done)* `tool/resource_manager/serialization/godot.go` — added `mapTiles` to `packGodotAtlases` (8th atlas in the Godot tree). Output is `mapTiles.png` + `mapTiles.offsets.json` (37 Type-2 entries, names `0.png`..`36.png`). Note: this atlas turned out to be the **world-map overview** assets (city streets, enemy/friendly cafe buildings, "BACK" UI buttons), not cafe-interior floor tiles — kept anyway as Phase 5 prerequisite.
- *(done)* `tool/cctpacker/cct_file/packed_texture.go` — bug fix: `WritePackedTexture` had a hard `src_img.(*image.NRGBA)` type assertion + `log.Fatal("Could not cast image to NRGBA")` that fired on opaque RGB PNGs. `mapTiles/0.png` is the first such PNG to hit the pipeline. Replaced with `imaging.Clone(src_img)` which always returns `*image.NRGBA`. Idiomatic for this codebase (already imported), zero behavior change for existing RGBA inputs.
- *(done)* `godot/scripts/cafe_renderer.gd` — new `class_name CafeRenderer` (RefCounted). `render(parent, cafe_dict, atlases) -> int` walks `Cafe.Tiles[]` and emits `Sprite2D` children via three passes (floor → walls → furniture) with `z_index` layering. `TILE_W=TILE_H=50` from `Cafe.Tiles` coordinate inspection.
- *(done)* `godot/scripts/main_scene.gd` — `assemble(mode)` dispatch: `"grid"` preserves the existing 27-piece boxer-human grid (so the existing 15th validator check keeps passing); `"cafe"` is the new path. Default boot is `"cafe"`. `_assemble_cafe` loads `playerCafe.caf` via Phase 3 `LegacyLoader.parse_cafe_bytes`, instantiates the furniture atlas, calls `CafeRenderer.render(self, cafe_dict, atlases)`.
- *(done)* `godot/validate_assets.gd` — 16th check `_validate_cafe_render`. Asserts 1925 floor sprites (35×55 grid), ≥23 cafe-object sprites, every `Sprite2D` has a non-null texture. Final tally: **1925 floor + 29 objects = 1954 total**, 16/16 PASSED.
- *(RE finding)* **Spec hypothesis was wrong.** The Phase 4 design spec assumed `Cafe.Tiles[].U1` would map to a `mapTiles` atlas region via some indirection LUT (Tasks 9-10 in the plan). Visual inspection of the actual `mapTiles/*.png` source files revealed mapTiles is the **world-map overview atlas** — for the Phase 5 city-street view of friendly + enemy cafe buildings, NOT cafe-interior floor textures. The actual answer: cafe tiles, walls, and furniture all share the existing `furniture` atlas, with a direct `region = "%d.png" % U1` mapping (no indirection). Verified by matching every observed Tile.U1 value (29-113) against `furniture.offsets.json` keys + visual inspection of `furniture/51.png`, `/68.png`, `/113.png` — all isometric grass/floor diamond tiles. So `_atlas_region_for_tile_u1(u1)` returns `"%d.png" % u1` directly; the 45-min empirical-RE budget was spent on hypothesis testing that produced a better answer than the original spec assumed.
- *(closed in Session 1.5)* Coordinate system: was Cartesian `(tx*50, ty*50)` in Session 1, producing a zigzag/staircase pattern at long pavement edges because the diamond sprite contours didn't tessellate. Session 1.5 corrected this to proper iso projection: `screen_x = (tx-ty) * 50`, `screen_y = (tx+ty) * 25` with TILE_W=100/TILE_H=50 stride. Tile diamond edges now chain into continuous lines. CafeRenderer's three passes + CustomerSystem's seat positioning + main.tscn's Camera2D all updated. New cafe pixel bounds: x ∈ [-2700, 1864], y ∈ [0, 2305]. Camera at (-418, 1153) zoom 0.25 fits the full cafe in the default 1152×648 viewport.
