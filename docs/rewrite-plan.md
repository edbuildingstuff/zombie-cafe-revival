...
8. *(mostly done — FriendCafe still pending)* Check in real binary fixtures under `tool/file_types/testdata/`. Landed: two real `Cafe` fixtures (`playerCafe.caf`, `BACKUP1.caf`) extracted from a physical ARM Samsung device running the legacy patched APK; both round-trip byte-identically via `TestRealCafeFixturesRoundTrip`. Also landed: two real `SaveGame` fixtures (`globalData.dat`, `BACKUP1.dat`) round-tripping via `TestRealSaveGameFixturesRoundTrip` — the existing `SaveGame` parser was extended with a `Trailing []byte` preservation field because the real device files carry ~1 KB of additional data past the known struct fields (probably an extended character/friend list near offset 0x265 in globalData.dat — visible in hex as `Lionel`, `Odie`, `Krystal`, etc.). Decoding that trailing section structurally is deferred; byte-preservation is sufficient for the round-trip contract. Still pending: a real `FriendCafe` fixture, which requires playing through a raid on the device to populate the `raidCafe*.dat` and `raidEnemies*.dat` slots (currently 2 bytes each = empty). Getting the game to boot on the modern device required building `libZombieCafeExtension.so` from source via the NDK's bundled CMake+Ninja and adding a runtime patch that NOPs out `Java_com_capcom_zombiecafeandroid_SoundManager_setEnabled` to dodge a null-pointer race triggered by modern Android's broadcast-receiver lifecycle (see the `lib/cpp: patch SoundManager.setEnabled` commit and `src/lib/cpp/ZombieCafeExtension.cpp`).
...
3. *(in progress)* Opaque binary game data. Still pending: `constants.bin.mid`, `enemyLayouts.bin.mid`, `font3.bin.mid`. Done this phase:
