...
3. *(in progress)* Opaque binary game data. `constants.bin.mid`, `cookbookData.bin.mid`, `enemyCafeData.bin.mid`, `enemyItems.bin.mid`, `enemyLayouts.bin.mid`, `font3.bin.mid`, `strings_amazon.bin.mid`, `strings_google.bin.mid` still have no parsers. **`enemyItemData.bin.mid` is done:** `tool/file_types/enemy_item_data.go` with `ReadEnemyItemData` / `WriteEnemyItemData`, byte-identical round-trip test against the real file (321 bytes = 1-byte count header `0x14=20` + `20 × 16` per-item attribute records), and an in-memory fixture test with non-default values. Wired through `BuildGodotAssets` via a new `deserializeEnemyItemDataForGodot` helper that emits `<out>/assets/data/enemyItemData.json` (the 16-byte records serialize as base64 strings via Go's default `[]byte` JSON handling — functional and Godot-side-decodable via `Marshalls.base64_to_raw()` when a consumer needs it; can be swapped to numeric arrays later if readability matters). Per-item column meanings (item stats? tier progression? upgrade costs?) are not interpreted — same preservation philosophy as Phase 0b. The other 8 opaque files still pending. Note: `constants.bin.mid` was attempted first but has a more complex mixed-endian structure (9789 bytes, not divisible by 4, ints appear big-endian at the start but the layout past the first block isn't obvious from hex dumps alone); deferred to a longer-scope session.
